...
/**
 * The JTypeSpace class is where the game takes place.  This class extends JPanel and implements
 * Runnable in order to allow animation to occur in the game.  This class calls all the methods
...
public class JTypeSpace extends JPanel implements Runnable
{
	...
	/**
	 * Starts the game.  Any previous key listeners are moved before creating a new key listener
	 * that is associated with the current space ship.  This is useful for when the game is reset
	 * or when the player dies and a new space ship is created.  At the end of the method the 
	...
	/**
	 * The paint method draws all the objects in the game to screen.  Various methods are called inside
	...
	public void paint(Graphics g)
	{
		...
		if(gameOver == false)
		{
			...
			{
				/*
				 * The method in the if statement makes it possible for a message to be on a the screen for a period
				 * of time even though the screen is refreshed 30 times per second.
				 */
				if(holdForMessage(framesPerSecond * 3))
				{
					g2.setFont(font);
					g2.drawString("Level "+ Integer.toString(curLevel + 1) + " Complete", 560, 300);
				}
				else
				{
					nextLevel();
	...
	/**
	 * This method controls the speed of the animation and calls the <code>repaint</code>
	 * method that will draw the game's objects to the screen.  Once the game is over it will
	...
	/**
	 * This method allows for messages to be displayed on the screen for a certain period of time
	 * even though the screen is refreshed very rapidly.  Each time the method is called an iterator
	 * is increased until it reaches the length specified in the parameter.  The method will then 
	...
	/**
	 * Draws the player's stats to the screen.  This includes the player's health, ammo,
	...
	{	
		...
		g.drawRect(405 + fm.stringWidth("Health: "), y - fm.getAscent(), 100, fm.getAscent());
		g.fillRect(405 + fm.stringWidth("Health: "), y - fm.getAscent(), ship.getHP(), fm.getAscent());
		g.drawString("Level " + Integer.toString(curLevel + 1), 700, y);
	}
	
	/**
	 * Moves the game to the next level if there is another level available.  Otherwise
