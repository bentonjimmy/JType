...
/**
 * Extends the AbstractSpaceAlien to create a space alien.  The alien will only move so far to the left 
 * but will move up or down to be in line with the space ship.  The alien shoots less often than the
...
public class SpaceAlien3 extends AbstractSpaceAlien 
{
	public SpaceAlien3(JTypeSpace space, int x, int y)
	{
		...
		score = 20;
		hp = 5;
		ImageIcon icon = new ImageIcon(this.getClass().getResource("LevelOneC.png"));
		image = icon.getImage();
	...
	public void draw(Graphics2D g) 
	{
		if(visible)
		{
			...
			{
				ImageIcon icon = new ImageIcon(this.getClass().getResource("LevelOneC1.png"));
				image = icon.getImage();
				shootVar++;
			}
			else if(shootVar >= 10 && shootVar < 20)
			{
				ImageIcon icon = new ImageIcon(this.getClass().getResource("LevelOneC2.png"));
				image = icon.getImage();
				shootVar++;
			}
	...
	protected int shootInc; //controls how often the alien will shoot it's gun
}
