...
/**
 * This class represents the players space ship.  The player controls the movement
 * and shooting of the space ship.  The ship has an HP(Hit points) associated with it
 * which determine how much life the player has left.  The ship is represented on screen
 * by an image of a ship.  The size of this image is used to determine if the player
...
{	
	/**
	 * Constructor for the space ship.  The <code>space</code> parameter is used to
	...
	public SpaceShip(JTypeSpace space)
	{
		...
		ImageIcon icon = new ImageIcon(this.getClass().getResource("SpaceShipMed.png"));
		image = icon.getImage();
	...
	/**
	 * This method draws the ship and all of it's gun shots onto the associated 
	 * <code>space</code>.  Only visible shots are drawn to the screen.  If a shot
	 * is no longer visible it will be removed from the <code>shots</code> List.
	 * 
	...
	/**
	 * Moves the ship up the screen by five.  If the player is already moving in that
	...
	/**
	 * Moves the ship down the screen by five.  If the player is already moving in that
	 * direction then the speed at which they are moving will increase.
	 */
	...
	/**
	 * Moves the ship right by five.  If the player is already moving in that
	 * direction then the speed at which they are moving will increase.
	 */
	...
	/**
	 * This method first determines if the current gun has ammo to shoot or if the 
	 * gun is a BlasterGun.  If the gun has ammo then a new shot is added to the 
	 * <code>shots</code> List.  If there is no ammo in the gun, the gun is switched
	...
	/**
	 * Returns the Shot located at position <code>i</code> of the shots List.
	 * If <code>i</code> doesn't exist in the List then a <code>null</code>
	 * value is returned.
	 * 
	...
	/**
	 * Removes the Shot at position <code>i</code> from the shots List.  If
	...
	/**
	 * Returns a new rectangle that represents the current location and size of the
	 * space ship.  The Rectangle is used to determine if the space ship has collided
	 * with any of the objects on the screen.
	 * 
	 * @return - a Rectangle representing the size and location of the ship.
	 */
	...
	/**
	 * This method is used to give damage to the ship.  The amount specified
	 * in the parameter is deducted from the player's hp.  If the hp goes to 
	...
	/**
	 * Attempts to add a new Gun to the GunKit.  If the ship already has the Gun
	 * then the player received 30 additional pieces of ammo for that Gun.  If it
	 * is a Gun the player does not already have then the Gun is added to the GunKit
	 * and a new Button with the name of the Gun is added to the player's tool bar.
	 *
